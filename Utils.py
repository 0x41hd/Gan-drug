...
import json
# fix: 'bunch' is unmaintained and won't install on Python 3.11+ (Colab).
# Try bunch, then munch (its maintained fork), then a tiny built-in fallback.
try:
    ...
    try:
        ...
        class Bunch(dict):
            """Minimal attribute-access dict (fallback for bunch/munch)."""
...
try:
    ...
    DrawingOptions = None  # not used anywhere in this project
...
def secondsToStr(t):
    return "%d:%02d:%02d.%03d" % \
        reduce(lambda ll, b: divmod(ll[0], b) +
...
def diversity(smiles_A, smiles_B=None):
    ...
    if smiles_B is None:  # fix: use "is None" not "== None"
...
def external_diversity(file_A, file_B):
    ...
    if file_B is None:  # fix: use "is None" not "== None"
        for ii in range(len(fps_A)):
            for xx in range(len(fps_A)):
                ts = 1 - DataStructs.TanimotoSimilarity(fps_A[ii], fps_A[xx])
                td += ts
...
def ccc(y_true, y_pred):
    ...
    den = K.sum(K.square(y_true-K.mean(y_true))) + K.sum(K.square(y_pred-K.mean(y_pred))
                                                         ) + K.int_shape(y_pred)[-1]*K.square(K.mean(y_true)-K.mean(y_pred))
    return num/den
...
def load_config(config_file, property_identifier):
    ...
    with open(config_file, 'r') as config_file:
        ...
        config.checkpoint_dir = os.path.join('experiments', property_identifier + '-' + exp_time,
                                             config.exp_name, 'checkpoints', '')  # fix: removed Windows-only backslashes
...
def reading_csv(config, property_identifier):
    ...
    if property_identifier == 'bbb':
        ...
        idx_smiles = 0
        # fix: column 1 is "CID" (non-numeric); pIC50 is column 4
        idx_labels = 4
...
def data_division(config, smiles_int, labels, cross_validation, model_type, descriptor):
    ...
    idx_test = np.array(random.sample(range(0, len(smiles_int)), math.floor(
        config.percentage_test*len(smiles_int))))
    ...
    if cross_validation:
        ...
        idx_val = np.array(random.sample(range(0, len(train_val_set)), math.floor(
            config.percentage_test*len(train_val_set))))
...
def cv_split(data, config):
    ...
    data_cv = list(cross_validation_split.split(
...
#        sd_train = data[l][3]
        ...
        for c in range(len(predictions[0])):
            predictions[l, c] = (max_train - min_train) * \
                predictions[l, c] + min_train
...
def regression_plot(y_true, y_pred):
    ...
    ax.plot([np.min(y_true), np.max(y_true)], [
            np.min(y_true), np.max(y_true)], 'k--', lw=4)
    ax.set_xlabel('True')
...
def sample_data_from_generator(generator, autoencoder, vocab, z_dim, n, run_folder, save):
    ...
    generated_smiles = []
    # transforms fake data into SMILES
...
def update_data_feedback_gan_multi_obj(previous_data, previous_smiles, gen_smiles, gen_data, predictor, property_identifier, threshold, info):
    ...
    # LogP
    # fix: pass None for property-only evaluations
    logp = evaluate_property(None, gen_smiles, 'logP')
    print('\n\n\n log P \n\n\n\n')
    aux_smiles = []
    idx_save = []
    ...
    print('\n\n\n\n SA scoer \n\n\n\n')
    # fix: pass None for property-only evaluations
    sascore = evaluate_property(None, gen_smiles, 'sascore')
    ...
    # pIC50
    print('\n\n\n PIC50 \n\n\n\n')
    pIC50, og_idx = evaluate_property(
        predictor, gen_smiles, property_identifier)
    ...
    eval_previous_smiles, _ = evaluate_property(
        predictor, previous_smiles, property_identifier)
    ...
    if info == 'max':
        output = np.vstack((sorted_previous_data[n:sorted_previous_data.shape[0], :], np.squeeze(
            np.array(sorted_gen_data[len(sorted_gen_data)-n:len(sorted_gen_data)]))))

        output_smiles = sorted_previous_smiles[n:len(
            sorted_previous_smiles)] + sorted_gen_smiles[len(sorted_gen_smiles)-n:len(sorted_gen_smiles)]
...
def update_data_feedback_gan(previous_data, previous_smiles, gen_smiles, valid_generated_data, predictor, property_identifier, threshold, info):
    ...
    eval_smiles, og_idx = evaluate_property(
        predictor, gen_smiles, property_identifier)
    ...
    if info == 'max':
        ...
        output = np.vstack((sorted_previous_data[n:sorted_previous_data.shape[0], :], np.squeeze(
            np.array(sorted_new_data_[len(sorted_new_data_)-n:len(sorted_new_data_)]))))

        output_smiles = sorted_previous_smiles[n:len(
            sorted_previous_smiles)] + new_smiles[len(new_smiles)-n:len(new_smiles)]
        ...
        for idx, j in enumerate(eval_smiles):
            if j < threshold:
                if gen_smiles[idx] not in sorted_previous_smiles:
                    ...
                    sorted_previous_data = np.vstack(
                        (sorted_previous_data, np.array(valid_generated_data[idx:idx+1][0])))
        ...
        output_smiles = new_smiles + \
            sorted_previous_smiles[0:len(sorted_previous_smiles)-n]
...
def plot_hist_both(prediction_unb, prediction_b, property_identifier):
    ...
    if property_identifier == 'jak2' or property_identifier == "kor" or property_identifier == "a2d":
        ...
        print("Max of pIC50: (UNB,B)", np.max(
            prediction_unb), np.max(prediction_b))
        print("Mean of pIC50: (UNB,B)", np.mean(
            prediction_unb), np.mean(prediction_b))
        print("Min of pIC50: (UNB,B)", np.min(
            prediction_unb), np.min(prediction_b))
        ...
        print("Max of SA score: (UNB,B)", np.max(
            prediction_unb), np.max(prediction_b))
        print("Mean of SA score: (UNB,B)", np.mean(
            prediction_unb), np.mean(prediction_b))
        print("Min of SA score: (UNB,B)", np.min(
            prediction_unb), np.min(prediction_b))
        ...
        print("Max of QED: (UNB,B)", np.max(
            prediction_unb), np.max(prediction_b))
        print("Mean of QED: (UNB,B)", np.mean(
            prediction_unb), np.mean(prediction_b))
        print("Min of QED: (UNB,B)", np.min(
            prediction_unb), np.min(prediction_b))
        ...
        print("Average of log_P: (UNB,B)", np.mean(
            prediction_unb), np.mean(prediction_b))
        print("Median of log_P: (UNB,B)", np.median(
            prediction_unb), np.median(prediction_b))
    ...
    # fix: shade= renamed to fill= in seaborn 0.12
    ax = sb.kdeplot(v1, fill=True, color='b')
    # fix: shade= renamed to fill= in seaborn 0.12
    sb.kdeplot(v2, fill=True, color='r')
