...
def load_config(config_file,property_identifier):
    ...
    with open(config_file, 'r') as config_file:
        ...
        config.checkpoint_dir = os.path.join('experiments', property_identifier + '-' + exp_time, config.exp_name, 'checkpoints', '')  # fix: removed Windows-only backslashes
...
def update_data_feedback_gan_multi_obj(previous_data, previous_smiles, gen_smiles, gen_data, predictor, property_identifier, threshold, info):
    ...
    # LogP
    logp = evaluate_property(None, gen_smiles, 'logP')  # fix: pass None for property-only evaluations
    print('\n\n\n log P \n\n\n\n')
    aux_smiles = []
    idx_save = []
    ...
    #SA score
    print('\n\n\n\n SA scoer \n\n\n\n')
    sascore = evaluate_property(None, gen_smiles, 'sascore')  # fix: pass None for property-only evaluations
...
def plot_hist_both(prediction_unb,prediction_b, property_identifier):
    ...
    ax = sb.kdeplot(v1, fill=True, color='b')  # fix: shade= renamed to fill= in seaborn 0.12
    sb.kdeplot(v2, fill=True, color='r')  # fix: shade= renamed to fill= in seaborn 0.12
