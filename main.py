...
autoencoder = AE(path_model, decoder_input_shape, latent_dim, lstm_units, output_dim, batch_norm,
                 batch_norm_momentum, noise_std, numb_dec_layer, emb_dim, vocab.vocab_size, vocab.max_len)

autoencoder.load_autoencoder_model(path_model+'model--86--0.0013.hdf5')

# pass the data through the autoencoder
x_latent = autoencoder.smiles_to_latent_model.predict(
    X_train)  # ---> this will be the real data to train the GAN

# Load Predictor Object
predictor = Predictor(configReinforce, vocab, 'dnn',
                      'SMILES', property_identifier, False)
...
gan = WGANGP(input_dim, critic_layers_units, critic_lr, critic_dropout, gp_weight, z_dim, generator_layers_units,
             generator_batch_norm_momentum, generator_lr, generator_dropout, batch_size, critic_optimizer, generator_optimizer)
...
valid_smiles_before, perc_valid_before, _ = gan.sample_valid_data(
    1000, run_folder, True)
predictions_before, _ = evaluate_property(
    predictor, valid_smiles_before, property_identifier)
...
gan.train_feedbackGAN(data, smiles_raw, batch_size, epochs, run_folder, autoencoder, vocab, predictor,
                      n_to_generate, threshold, info, property_identifier, print_every_n_epochs=50, critic_loops=5)

valid_smiles_after, perc_valid_after, _ = gan.sample_valid_data(
    1000, run_folder, True)
predictions_after, _ = evaluate_property(
    predictor, valid_smiles_after, property_identifier)
...
predictions_before = np.array(predictions_before).astype(
    float)  # fix: np.float removed in NumPy 1.24
print(predictions_after)
print(type(predictions_after))
diff = plot_hist_both(predictions_before,
                      predictions_after, property_identifier)
