...
class Vocabulary:
    ...
    def update_vocab(self, smiles):
        ...
        for i, sm in enumerate(smiles):
            ...
            sm = sm.replace('Br', 'R').replace(
                'Cl', 'L').replace('Se', 'E').replace('Si', 'X')
        ...
        print('Number of unique characters in the vocabulary: {} '.format(
            len(unique_chars)))
    ...
    def tokenize(self, smiles):  # Transforms a List of SMILES Strings into a list of tokenized SMILES (where each tokenized SMILES is in itself a list)
        ...
        for idx, smile in enumerate(smiles):
            # regex = '(\[[^\[\]]{1,6}\])'
            # print(smile)
            smile = smile.replace('Br', 'R').replace(
                'Cl', 'L').replace('Se', 'E').replace('Si', 'X')
            ...
            if len(smile_tok) == self.max_len:
                # fix: removed per-molecule print() that floods output on large datasets
                list_tok_smiles.append(smile_tok)
                og_idx.append(idx)
            else:
                pass  # SMILES too long after tokenization — silently skip
    ...
    def decode(self, encoded_smiles):
        ...
        for e_smile in encoded_smiles:
            ...
            smile_str = smile_str.replace('R', 'Br').replace(
                'L', 'Cl').replace('E', 'Se').replace('X', 'Si')
    ...
    def one_hot_encoder(self, smiles_list):
        ...
        # smiles_one_hot = []
        smiles_one_hot = np.zeros(
            (len(smiles_list), self.max_len, self.vocab_size), dtype=np.int8)
