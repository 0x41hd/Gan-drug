...
class WGANGP():
    ...
    def get_optimizer(self, optimizer, lr):
        if optimizer == 'adam':
            opti = Adam(learning_rate=lr, beta_1=0, beta_2=0.9)  # fix: lr= deprecated in TF 2.11+ #valores defaul do paper original wgan-gp
        return opti
    ...
    def sample_valid_data(self, n, run_folder, save):
        ...
        #while (aux <n):
        temp = aux
        MAX_ATTEMPTS = 50  # fix: guard against infinite loop if AE produces no valid SMILES
        attempt = 0
        while temp > 0 and attempt < MAX_ATTEMPTS:
            attempt += 1
            #if aux == n &&:
    ...
    def plot_model(self, run_folder):
    	plot_model(self.critic, to_file=os.path.join(run_folder, 'viz/critic.png'), show_shapes=True, show_layer_names=True)  # fix: moved closing paren of os.path.join
    	plot_model(self.generator, to_file=os.path.join(run_folder, 'viz/generator.png'), show_shapes=True, show_layer_names=True)  # fix: moved closing paren of os.path.join