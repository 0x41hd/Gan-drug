...
class Predictor():
    def __init__(self, config, vocab, model_type, descriptor_type, property_identifier, load):
        self.dropout = 0.3
        self.path_predictor = "predictor_weigths_" + \
            property_identifier+"/"+property_identifier
        # fix: derive the embedding input dimension from the actual Vocabulary so
        # that the predictor's Embedding layer always matches the encoded SMILES
        # indices. Falling back to tokens_table only if vocab is unavailable.
        if vocab is not None and hasattr(vocab, 'vocab_size'):
            self.n_table = vocab.vocab_size
        else:
        ...
        # fix: only load pre-trained weights when load=True. When training from
        # scratch (load=False) we build 5 fresh models instead of crashing on
    ...
    def build_model(self):
        ...
        # fix: removed deprecated input_length (Embedding) and input_shape (LSTM)
        # args for Keras 3 compatibility. Architecture is identical.
        my_model.add(Embedding(self.n_table, self.n_units))
        my_model.add(
            LSTM(self.n_units, return_sequences=True, dropout=self.dropout))
    ...
    def train_model(self, data, searchParam):
        ...
        opt = Adam(learning_rate=self.config.lr, beta_1=self.config.beta_1,
                   beta_2=self.config.beta_2, amsgrad=False)  # fix: lr= deprecated in TF 2.11+
        ...
        self.model.compile(loss=self.config.loss_criterium,
                           optimizer=opt, metrics=[mse, r_square, rmse, ccc])
        ...
        if self.searchParam:
            ...
            metrics = self.model.evaluate(
                x=self.data[2], y=self.data[3])  # test
            ...
            if self.descriptor_type == 'SMILES':
                ...
                file = [i.rstrip().split(',') for i in open(
                    'grid_results_'+self.descriptor_type+'.csv').readlines()]
                file.append(values)
                file = pd.DataFrame(file)
                file.to_csv('grid_results_'+self.descriptor_type +
            ...
            # fix: ensure the checkpoint directory exists before writing
            ...
            # fix: Keras 3 requires .weights.h5 suffix
            self.model.save_weights(str(filepath + ".weights.h5"))
            print("Saved model to disk")

    def build_models(self):
        # fix: build 5 fresh (untrained) models for training-from-scratch mode.
        loaded_models = []
        for i in range(5):
            my_model = self.build_model()
            print("Model " + str(i) + " built (untrained)!")
    ...
    def load_models(self):
        ...
        for i in range(5):
            my_model = self.build_model()
            # fix: Keras 3 saves weights as '.weights.h5'; legacy used '.hdf5'.
            # Try the modern suffix first, fall back to the legacy one.
            base = self.path_predictor + "model" + str(i)
            for suffix in (".weights.h5", ".hdf5", ".h5"):
                try:
                    my_model.load_weights(base + suffix)
                    print("Model " + str(i) +
                          " loaded from disk (" + suffix + ")!")
                    break
                except (OSError, IOError, ValueError):
                    continue
            else:
                raise FileNotFoundError(
                    f"No weights found for predictor model {i} at {base}.weights.h5/.hdf5/.h5")
            loaded_models.append(my_model)
    ...
    def evaluator(self, data):
        ...
        for m in range(len(self.loaded_models)):
            ...
            self.loaded_models[m].compile(
                loss=self.config.loss_criterium, optimizer=opt,  metrics=[mse, r_square, rmse, ccc])
