...
class Autoencoder:
    ...
    def build_smiles_to_latent_model(self):
        ...
        for i in range(self.numb_dec_layer):
            if self.numb_dec_layer == 1:
                ...
                encoder = Bidirectional(
                    LSTM(self.lstm_units // 2, return_state=True, name='encoder'+str(i)+'_LSTM'))

                x, state_h, state_c, state_h_reverse, state_c_reverse = encoder(
                    x)
                ...
                encoder = Bidirectional(LSTM(
                    self.lstm_units // 2, return_sequences=True, return_state=True, name='encoder'+str(i)+'_LSTM'))
                ...
                if self.batch_norm:
                    x = BatchNormalization(
                        momentum=self.batch_norm_momentum, name='BN_'+str(i))(x)
                ...
                _, state_h2, state_c2, state_h2_reverse, state_c2_reverse = encoder2(
                    x)
        ...
        # states c and h and then the reversed states
        ...
        if self.batch_norm:
            states = BatchNormalization(
                momentum=self.batch_norm_momentum, name='BN_'+str(i+1))(states)

        # neckouptus = latent representation
        latent_representation = Dense(
            self.latent_dim, activation="relu", name="Dense_relu_latent_rep")(states)

        if self.batch_norm:
            latent_representation = BatchNormalization(
                momentum=self.batch_norm_momentum, name='BN_latent_rep')(latent_representation)

        # Adding Gaussian Noise as a regularizing step during training
        latent_representation = GaussianNoise(
            self.noise_std, name='Gaussian_Noise')(latent_representation)

        self.smiles_to_latent_model = Model(
            encoder_inputs, latent_representation, name='smiles_to_latent_model')

        with open('smiles_to_latent.txt', 'w') as f:

            self.smiles_to_latent_model.summary(
    ...
    def build_latent_to_states_model(self):
        ...
        for dec_layer in range(self.numb_dec_layer):  # 2
            ...
            h_decoder = Dense(
                self.lstm_units, activation="relu", name=name)(latent_input)
            ...
            if self.batch_norm:
                ...
                h_decoder = BatchNormalization(
                    momentum=self.batch_norm_momentum, name=name)(h_decoder)

                name = "BN_c_" + str(dec_layer)
                c_decoder = BatchNormalization(
                    momentum=self.batch_norm_momentum, name=name)(c_decoder)
    ...
    def build_states_to_smiles_model(self):
        ...
        # list that will have all the inputs to this model: decoder_inputs + reconstructed states (from 2nd model)
        inputs = []
        ...
        for dec_layer in range(self.numb_dec_layer):
            ...
            # LSTM layer
            decoder_lstm = LSTM(
                self.lstm_units, return_sequences=True, name="Decoder_LSTM_" + str(dec_layer))

            x = decoder_lstm(x, initial_state=[state_h, state_c])

            if self.batch_norm:
                x = BatchNormalization(
                    momentum=self.batch_norm_momentum, name="BN_decoder_"+str(dec_layer))(x)

            # if smt >0 Time Distributed

        # Dense layer that will return probabilities
        outputs = Dense(self.output_dim, activation="softmax",
                        name="Decoder_Dense")(x)

        self.states_to_smiles_model = Model(
            inputs=inputs, outputs=[outputs], name="states_to_smiles_model")
        with open('states_to_smiles.txt', 'w') as f:

            self.states_to_smiles_model.summary(
                print_fn=lambda x: f.write(x + '\n'))

    def build_model(self):
        # same as the smiles_to_latent_model input
        encoder_inputs = Input(shape=(None,), name="encoder_inputs")
        # same as the input to the 3rd model. It's needed for teacher's forcing
        decoder_inputs = Input(shape=self.input_shape, name="decoder_inputs")
        ...
        # decoder inputs for teacher's forcing and x will be the reconstructed states
        x = [decoder_inputs] + x
        x = self.states_to_smiles_model(x)

        # Full model
        self.model = Model(inputs=[encoder_inputs, decoder_inputs], outputs=[
                           x], name="Autoencoder")
        print(self.model.summary)
    ...
    def fit_model(self, dataX, dataX2, dataY, epochs, batch_size, optimizer):
        ...
        if optimizer == 'adam':
            ...
            self.optimizer = Adam(
                learning_rate=0.001, beta_1=0.9, beta_2=0.999, amsgrad=False, clipvalue=3)

        # Callbacks
        checkpoint_dir = self.path
        # checkpoint_file = (checkpoint_dir + "%s--{epoch:02d}--{val_loss:.4f}--{lr:.7f}.hdf5" % model_name)
        # fix: Keras 3 requires '.weights.h5' for weights-only checkpoints.
        # save_weights_only=True keeps this compatible with load_weights().
        checkpoint_file = (
            checkpoint_dir + "model--{epoch:02d}--{val_loss:.4f}.weights.h5")
        checkpoint = ModelCheckpoint(
            checkpoint_file, monitor="val_loss", mode="min", save_best_only=True, save_weights_only=True)
        ...
        results = self.model.fit([dataX, dataX2], dataY, epochs=self.epochs, batch_size=self.batch_size,
                                 validation_split=0.1, shuffle=True, verbose=1, callbacks=callbacks_list)
    ...
    def build_sample_model(self):
       ...
       # new_config = config
        ...
        for layer in sample_model.layers:
            ...
            weights = self.states_to_smiles_model.get_layer(
...
if __name__ == "__main__":
    ...
    auto = Autoencoder('', input_shape, latent_dim, lstm_units, output_dim,
                       batch_norm, batch_norm_momentum, noise_std, numb_dec_layer)

    # Save model
    with open('Summary_autoencoder.txt', 'w') as fh:
        # Pass the file handle in as a lambda function to make it callable
        auto.model.summary(
            line_length=237, print_fn=lambda x: fh.write(x + '\n'))
