...
class Autoencoder:
    ...
    def fit_model(self,dataX, dataX2, dataY, epochs, batch_size, optimizer):
        ...
        checkpoint_file = (checkpoint_dir + "model--{epoch:02d}--{val_loss:.4f}.weights.h5")
        checkpoint = ModelCheckpoint(checkpoint_file, monitor = "val_loss", mode = "min", save_best_only = True, save_weights_only = True)
    ...
    def latent_to_smiles(self, latent, vocab):  #sample
        ...
        # fix: direct call is much faster than .predict() for single samples
        states = self.latent_to_states_model(latent, training=False)
        ...
        sample_vector = np.zeros(shape = (1,1,vocab.vocab_size), dtype=np.float32)
        sample_vector[0,0,vocab.char_to_int["G"]] = 1  #input char
        smiles = ""
        for i in range(vocab.max_len):
            # fix: calling the model directly is far faster than .predict() for
            # single-step inference (avoids per-call graph/tracing overhead that
            # made sampling extremely slow).
            pred = self.sample_model(sample_vector, training=False)
            idx = int(np.argmax(pred))
            char = vocab.int_to_char[idx]
            if char!= "A":
                smiles = smiles + char
                sample_vector = np.zeros((1,1,vocab.vocab_size), dtype=np.float32)
